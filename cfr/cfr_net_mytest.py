...
class cfr_net(object):
    ...
    def __init__(self, x, t, y_ , p_t, cfg, r_alpha, r_lambda, do_in, do_out, dims):
        self.variables = {}
        self.wd_loss = 0

        if cfg['nonlin'].lower() == 'elu':
            self.nonlin = tf.nn.elu
        else:
            self.nonlin = tf.nn.relu

        self._build_graph(x, t, y_ , p_t, cfg, r_alpha, r_lambda, do_in, do_out, dims)
    ...
    def _build_graph(self, x, t, y_ , p_t, cfg, r_alpha, r_lambda, do_in, do_out, dims):
        """
        ...
        if cfg['n_in'] == 0 or (cfg['n_in'] == 1 and cfg['varsel']):
            dim_in = dim_input
        if cfg['n_out'] == 0:
            if cfg['split_output'] == False:
                dim_out = dim_in+1
            else:
                dim_out = dim_in
        ...
        h_in = [x]
        for i in range(0, cfg['n_in']):
            if i==0:
                ''' If using variable selection, first layer is just rescaling'''
                if cfg['varsel']:
                    weights_in.append(tf.Variable(1.0/dim_input*tf.ones([dim_input])))
                else:
                    weights_in.append(tf.Variable(tf.random_normal([dim_input, dim_in], stddev=cfg['weight_init']/np.sqrt(dim_input))))
            else:
                weights_in.append(tf.Variable(tf.random_normal([dim_in,dim_in], stddev=cfg['weight_init']/np.sqrt(dim_in))))

            ''' If using variable selection, first layer is just rescaling'''
            if cfg['varsel'] and i==0:
                biases_in.append([])
        ...
        if cfg['normalization'] == 'divide':
            h_rep_norm = h_rep / safe_sqrt(tf.reduce_sum(tf.square(h_rep), axis=1, keep_dims=True))
        else:
            h_rep_norm = 1.0*h_rep

        ''' Construct ouput layers '''
        y, weights_out, weights_pred = self._build_output_graph(h_rep_norm, t, dim_in, dim_out, do_out, cfg)
        ...
        ''' Regularization '''
        if cfg['p_lambda']>0 and cfg['rep_weight_decay']:
            for i in range(0, cfg['n_in']):
                if not (cfg['varsel'] and i==0): # No penalty on W in variable selection
                    self.wd_loss += tf.nn.l2_loss(weights_in[i])

        ''' Imbalance error '''
        if cfg['use_p_correction']:
            p_ipm = self.p_t
        else:
            p_ipm = 0.5

        if cfg['imb_fun'] == 'mmd2_rbf':
            imb_dist = mmd2_rbf(h_rep_norm,t,p_ipm,cfg['rbf_sigma'])
            imb_error = r_alpha*imb_dist
        elif cfg['imb_fun'] == 'mmd2_lin':
            imb_dist = mmd2_lin(h_rep_norm,t,p_ipm)
            imb_error = r_alpha*mmd2_lin(h_rep_norm,t,p_ipm)
        elif cfg['imb_fun'] == 'mmd_rbf':
            imb_dist = tf.abs(mmd2_rbf(h_rep_norm,t,p_ipm,cfg['rbf_sigma']))
            imb_error = safe_sqrt(tf.square(r_alpha)*imb_dist)
        elif cfg['imb_fun'] == 'mmd_lin':
            imb_dist = mmd2_lin(h_rep_norm,t,p_ipm)
            imb_error = safe_sqrt(tf.square(r_alpha)*imb_dist)
        elif cfg['imb_fun'] == 'wass':
            imb_dist, imb_mat = wasserstein(h_rep_norm,t,p_ipm,lam=cfg['wass_lambda'],its=cfg['wass_iterations'],sq=False,backpropT=cfg['wass_bpt'])
            imb_error = r_alpha * imb_dist
            self.imb_mat = imb_mat # FOR DEBUG
        elif cfg['imb_fun'] == 'wass2':
            imb_dist, imb_mat = wasserstein(h_rep_norm,t,p_ipm,lam=cfg['wass_lambda'],its=cfg['wass_iterations'],sq=True,backpropT=cfg['wass_bpt'])
            imb_error = r_alpha * imb_dist
            self.imb_mat = imb_mat # FOR DEBUG
        else:
    ...
    def _build_output(self, h_input, dim_in, dim_out, do_out, cfg):
        h_out = [h_input]
        dims = [dim_in] + ([dim_out]*cfg['n_out'])

        weights_out = []; biases_out = []

        for i in range(0, cfg['n_out']):
            wo = self._create_variable_with_weight_decay(
                    tf.random_normal([dims[i], dims[i+1]],
                        stddev=cfg['weight_init']/np.sqrt(dims[i])),
                    'w_out_%d' % i, 1.0)
        ...
        weights_pred = self._create_variable(tf.random_normal([dim_out,1],
            stddev=cfg['weight_init']/np.sqrt(dim_out)), 'w_pred')
        bias_pred = self._create_variable(tf.zeros([1]), 'b_pred')

        if cfg['varsel'] or cfg['n_out'] == 0:
    ...
    def _build_output_graph(self, rep, t, dim_in, dim_out, do_out, cfg):
        ...
        if cfg['split_output']:
            ...
            y0, weights_out0, weights_pred0 = self._build_output(rep0, dim_in, dim_out, do_out, cfg)
            y1, weights_out1, weights_pred1 = self._build_output(rep1, dim_in, dim_out, do_out, cfg)
            ...
            y, weights_out, weights_pred = self._build_output(h_input, dim_in+1, dim_out, do_out, cfg)
